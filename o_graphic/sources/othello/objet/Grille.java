...
public class Grille implements Grid, Constantes {
    ...
    /** tour de départ représentant la valeur du joueur : au début, en dur c'est le joueur1 qui commence */
    ...
    public void initPions() {
		tab_cell[3][3].reverseChar(JOUEUR2);
		tab_cell[3][4].reverseChar(JOUEUR1);
		tab_cell[4][3].reverseChar(JOUEUR1);
		tab_cell[4][4].reverseChar(JOUEUR2);
    } // public void initPions()
	...
	for(int y=(this.getHeight()-1); y>(-1); y--) {
	    ...
	    for(int x=0; x<this.getWidth(); x++) { // pour chaque colonne
	...
	do {
	    ...
	    compteur++;
	    col_car++;
	} while(compteur != (this.getWidth()+1)); // afficher le caractère de la colonne tant que la colonne n'est pas égale au nb de colonnes totales
    ...
    public void miseAjourPositionsPossibles(int joueur_maj) {
		...
		for(int y=0;y<this.getHeight();y++) { // on scanne toutes les lignes
		    
		    for(int x=0; x<this.getWidth(); x++) { // on scanne toutes les colonnes
    ...
    public int evaluationGrille(int joueur_courant) {
		...
		// calculs des valeurs des Cell
		for (int y=0; y<this.getHeight(); y++){
		    for (int x=0; x<this.getWidth(); x++) {
				if (this.getCellAt(new Position(y,x)).quelJoueur() == joueur_courant) {
				    notrePosition += EVAL[y][x];
				    saMobilite += mobiliteTableau;
				} else if (this.getCellAt(new Position(y,x)).quelJoueur() == -joueur_courant) {
				    saPosition += EVAL[y][x];
